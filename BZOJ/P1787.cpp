// http://www.lydsy.com/JudgeOnline/problem.php?id=1787
// [Ahoi2008]Meet 紧急集合

#include <algorithm>
#include <cstdio>
#include <vector>
using namespace std;

typedef pair<int, int> PII;

class Solution {
public:
    int N, M, root;
    int MAXB;
    vector<vector<int> > adj;
    vector<int> depth;
    vector<vector<int> > father;

public:
    Solution() {
        scanf("%d %d", &N, &M);
        root = 0;
        adj.resize(N);
        for (int i = 0; i < N-1; i++) {
            int x, y;
            scanf("%d %d", &x, &y);
            x--; y--;
            adj[x].push_back(y);
            adj[y].push_back(x);
        }
    }

    void preCompute() {
        MAXB = 0;
        int x = 1;
        while (x <= N) {
           MAXB++;
           x = x * 2;
        }

        depth.resize(N);
        father.resize(N);
        for (int i = 0; i < N; i++) {
            father[i].resize(MAXB, -1);
        }

        vector<bool> visited(N, false);
        dfs(root, -1, visited, 0);

        binaryLift();
    }

    void dfs(int x, int f, vector<bool>& visited, int d) {
        depth[x] = d;
        father[x][0] = f;
        visited[x] = true;
        for (int i = 0; i < (int)adj[x].size(); i++) {
            int u = adj[x][i];
            if (!visited[u]) dfs(u, x, visited, d+1);
        }
    }

    void binaryLift() {
        for (int j = 1; j < MAXB; j++) {
            for (int i = 0; i < N; i++) {
                if (father[i][j-1] != -1) {
                    father[i][j] = father[father[i][j-1]][j-1];
                }
            }
        }
    }

    int findLCA(int u, int v) {
        if (depth[u] < depth[v]) swap(u, v);
        for (int b = MAXB-1; b >= 0; b--) {
            if (father[u][b] == -1) continue;
            if (depth[father[u][b]] >= depth[v]) {
                u = father[u][b];
            }
        }

        if (u == v) return u;

        for (int b = MAXB-1; b >= 0; b--) {
            if (father[u][b] == -1) continue;
            if (father[u][b] != father[v][b]) {
                u = father[u][b];
                v = father[v][b];
            }
        }
        return father[u][0];
    }

    PII solveTriplet(int u, int v, int w) {
        int ans = 0;
        int x = findLCA(u, v);
        ans = depth[u] + depth[v] - 2 * depth[x];
        int y = findLCA(x, w);
        ans += depth[x] + depth[w] - 2 * depth[y];
        return make_pair(ans, x);
    }

    PII solveOnce(int u, int v, int w) {
        vector<PII> candidates(3);
        candidates[0] = solveTriplet(u, v, w);
        candidates[1] = solveTriplet(u, w, v);
        candidates[2] = solveTriplet(v, w, u);
        sort(candidates.begin(), candidates.end());
        return candidates[0];
    }

    void solve() {
        for (int i = 0; i < M; i++) {
            int u, v, w;
            scanf("%d %d %d", &u, &v, &w);
            u--; v--; w--;
            PII ans = solveOnce(u, v, w);
            printf("%d %d\n", ans.second+1, ans.first);
        }
    }
};

int main() {
    Solution solution;
    solution.preCompute();
    solution.solve();
    return 0;
}
