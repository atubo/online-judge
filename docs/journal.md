...
有 $f(i,n,k)=\sum_{n'}c(n',i)f(i-1,n-n',k)[n'\le a_i (k-1)] + \sum_{n'}c(n',i)g(i-1,n-n',k)[a_i (k-1) < n' \le a_i k]$
...
若询问区间的前缀和最小值为 $m$ 则需要将$x_1=\lceil m/2 \rceil$
个右括号改成左括号，若原区间和为$s$ 则改动后为 $s+2 x_1$
须再将 $s/2 + x_1$ 个左括号改成右括号。
...
### 01/03
* [洛谷 P5153] 简单的函数: 考虑到有些数 不可能是其他数的
最小约数，比如6，预处理出数组 $A=[2,3,2,5,7,2,...]$ 和
$B=[2,3,4,5,7,8,...]$ 表示若 $n$ 可被 $A_0, A_1,...,A_{i-1}$
整除，但整除后的结果不能被 $A_i$ 整除，则 $f(n)=f(B_i)+1$
考虑 $f(2)\cdot f(3) \cdot ... f(n)$ 若 $2$ 到 $n$ 中不能被
$2$ 整除的数目为$n_1$ ,能被 $2$ 整除的数目为 $n_2$ ,则前一部分
的贡献为 $(f(2)+1)^{n_1}$  后以部分的贡献可由递归求解 $[2,n_2]$
因子从 $3$ 开始的结果得到。
...
基环时考虑到有一次回退的机会，处理下一个基环节点
时，传入回退将会遇到的节点值。这样可决定何时回退。
...
式中$f$ 是 $u$ 的父亲。另有 $ans(l,r,x)=ans(0,r,x)-ans(0,l-1,x)$
求解 $ans(0,r,x)$ 找到 $x$ 的最低祖先 $p$ 满足 $l_p \le r$
从$W_p$ 中扣除 不足部分并加上高度差的贡献即可
$ans(0,r,x)=W_p-(p-r-1)+(r+1)h$

### 12/04
* [洛谷 P3207] [HNOI2010]物品调度: 令 $g=\gcd(d,n)$
若固定 $y_i$ 变化 $x_i$ 则最终位置一定关于$g$ 同余。
用一个线段树维护哪个同余类还有空余位置，用一个线段森林
维护不同的同余类的空余位置。确定最终位置后可根据各联通块
的长度来计算最小移动次数。
...
### 11/25
* [洛谷 P4643] [国家集训队]阿狸和桃子的游戏: 将每个点的权值
乘2，再加上连接此点的所有边的权值。排序后二人必从高到低取值。
...
和 $dp(i,i,w)=\max_{k<i}{dp(i-1,k,w-a_i)}+b_i+w_{k,i}$
坑点是 $a_i$ 和 $b_i$ 均可能为负。
...
* [洛谷 P5025] [SNOI2017]炸弹: 利用倍增思想，令
$B(d, i)$ 表示以 $i$ 为起点经过不超过 $2^d$ 次链式
反应（包括自己）的爆炸范围，有 $B(d+1,i)=\max_{j\in B(d,i)} B(d,j)$
是一个区间极值问题。
...
1表示已有上横沿的地板向下穿越，2表示没有上横沿的地板
向下穿越。转移时用DFS即可。

### 10/17
* [洛谷 P4578] [FJOI2018]所罗门王的宝藏: 可转化为
二分图，要求节点权值和等于边的权值。显然若一侧所有点
的权值 $-d$ 另一侧所有点的权值 $+d$ 依然满足条件。所以可
任选一点开始令其权值为0，DFS看是否有矛盾。
...
不经过点 $u$ 的所有路径可分为3类，终点在 $u$ 左侧的，起点在
$u$ 右侧的，跨越 $u$ 的。前两类很好处理，第三类用平衡树处理，
对于 $u$ 到 $v$ 的边，把经过此边的最长距离在处理到 $u$ 时加入
平衡树，处理到 $v$ 时从平衡树中删除。
...
二遍DFS中快速从父节点传播到子节点的哈希方法。
* [洛谷 P4939] Agent2: 离散化 + 树状数组。

### 05/20
* [洛谷 P5236] 【模板】静态仙人掌: 学习了圆方树处理
仙人掌的技巧。
...
上的每个点求出向左和向右走的最大距离，用双指针
维护向左和向右的范围。用两个优先队列维护最大距离。
枚举基环上的每个点，枚举左右下三个方向看是否可使得
最大距离更优。

### 04/25
* [洛谷 P4719] 【模板】"动态 DP"&动态树分治:
完全照[题解](https://oi-wiki.org/dp/dynamic/)抄的。

### 04/20
* [洛谷 P3476] [POI2008]TRO-Triangles: 先数学推导一波，若
...
放用coupon的牛，容量为 $K$ , $Q_p$ 放不用coupon的牛，容量
为 $n-K$ 当两个队列都满时，对于牛 $i$ 若取有两种可能，
直接替换掉 $Q_p$ 内一个元素，或替换掉 $Q_c$ 内一个元素，
并用替换掉的元素替换掉 $Q_p$ 内个一个元素。取两者花费较小的
即可。处理完所有的牛后判断总花费是否小于$M$
...
### 02/08
* [洛谷 P3974] [TJOI2015]组合数学: 贪心的思想。
从上往下处理，每行从左往右。处理到第 $j$ 列时，
从第 $j$ 列开始往左取富裕值直到满足要求，若取完也
不能满足要求，则将答案加上剩余要求值。再将第
$j$ 列的富裕值加上 $A[i][j]$ 手写双向链表维护
富裕值队列。
...
向右移动时，最优解的右桥一定也向右移动。故可
用双指针的方法:轮番向右移动左指针和右指针，找出
最优解。为快速更新答案，用一个set 和两个优先队列
维护在点左侧的区间，覆盖点的区间，在点右侧的区间。
比较考验码力。用了无耻O2。
...
先将所有点按 $(x,y)$ 排序，从左到右扫描，用四个
线段树分别存储 $x+y$ 的最大值，$x+y$ 的最小值，
$y-x$ 的最大值， $y-x$ 的最小值。这样在从左到右
扫描的过程中每个点到它左方的点的最大和最小距离。
类似再从右到左扫描一遍即可。
...
再枚举它与其他直线的交点（每个交点对应一条抛物线）。
转移到去除所有经过此交点的直线的集合。发现无视
神秘指令也可AC。
* [洛谷 P1090] 合并果子: 试炼场水题。
...
暴力 + 找规律。
令 $G(n,m)$ 表示长度为 $n$ 段数为 $m$ 的权值和，
有 $G(n,m)=\sum_{k=1}^{n-1}F_k G(n-k,m-1)$
...
到低位试探。观察数据范围，
对子任务1-4与子任务5分别处理，对子任务1-4，令 $dp(i,j)$
表示进行到位置 $i$ 分成 $j$ 段的可行性。对子任务5，令
$dp(i,j)$ 表示进行到位置 $i$ 分成段数小于等于 $j$ 的可行性。
第一种情况暴力转移，第二种情况可用树状数组转移。

### 12/01
* [洛谷 P3816] [FJOI2017]树的平均路长问题:
直觉告诉我们两边越不平衡则平均路长越大。从此出发
先递归地求出根节点为红/黑，黑高度为 $h$ 时的
最大/最小节点数。对每个节点数 $n$ 枚举可能的黑高度，
并对两边子树进行最不平衡的配置，得出结果。

### 11/30
* [洛谷 P4495] [HAOI2018]奇怪的背包: 不难想到
可搭配出 $W_i$ 的充分必要条件是子集的gcd可整除
$W_i$ 但在这里纠缠在以P的质因数为基的向量空间的处理，
过于复杂了。其实此向量空间里的每个点都是P的一个因数，
直接用此表示处理即可。
...
第一问比较简单，从上到小分层DP。
第二问实际上是要求有下界的最小流。看题解
发现有比较巧妙的方法，不建超级源汇，只建
虚拟源汇，求出最大伪流，再拿总出度减之即可。
* [洛谷 P1203] [USACO1.1]坏掉的项链Broken Necklace:
试炼场水题。

### 11/19
* [洛谷 P2490] [SDOI2011]黑白棋: 不难想到此题
可转化为 $k$ 堆，每次可取1到 $d$ 堆的nim游戏。
注意必败条件为对每个二进制位，1的个数可被 $d+1$
整除。然后用数位DP处理，令 $dp(n,l)$ 表示长度为
$n$ ,处理到二进制位 $l$ 的结果。

### 11/16
* [洛谷 P3733] [HAOI2017]八纵八横: 照题解写的，
线性基 + 线段树分治。
* [洛谷 P3989] [SHOI2013]阶乘字符串: 又是照题解写的，
状压DP，注意有结论当 $n>21$ 时无解。
...
### 11/10
* [洛谷 P1058] 立体图: 按从后往前，从左往右，
从下往上的次序画每个方块就可以很自然的处理
掩盖关系。

### 11/09
* [洛谷 P4408] [NOI2003]逃学的小孩: 两遍DFS，
第二遍时对点 $u$ 传入走向父节点所能到达的最远
距离，和 $u$ 子树的最大和次大深度一起可求出以
$u$ 为汇合点的最大结果。

### 11/06
* [洛谷 P3973] [TJOI2015]线性代数: 网络流，
从源点向第 $i$ 个点连边，权值为 $\sum_j A_ij$
表示不选 $i$ 的代价，从第 $i$ 个点向汇点连边，
权值为 $C_i$ 表示选 $i$ 的代价，从第 $i$ 个点
向第 $j$ 个点连边，权值为 $A_ij$ 表示选 $i$
但不选 $j$ 的代价，答案为矩阵A的总权重减去最大流。
...
空位只可以向右移动大于等于0个位置或向左移动1个
位置。可很快列出DP式在 $O(NM)$ 时间内求解。
...
1时最小存活数为1否则为0，若它有叶子，则最小存活数为叶子数。
对于最大存活数，可发现对于每颗树的情况其实就是树的最大点
独立集，DP求出根选或不选的最佳方案，再在基环上跑一遍DP即可。
...
内的每个点走一遍DFS求出向下的期望距离。因为基环长度有限，
可用DP求出 $dp(u,s,d)$ 表示当前出发点为 $u$ 环上起始点为
$s$ 方向为 $d$ 的期望距离。再走一遍DFS即可。
...
### 08/25
* [洛谷 P3588] [POI2015]PUS: 看题解才会作。拓扑排序 + 用线段树对区间连边。
...
* [洛谷 P2662] 牛场围栏: P3951加强版，可用同余类最短路解决。
...
数位DP。求出 $F(x,k)$ 特定位数的整数中数字 $x$
出现了 $k$ 次的整数数目，对于位数为偶数的情况，
还须求出 $G(x_1, x_2)$ 数字 $x_1$ , $x_2$ 均出现
一半的整数数目，对正好出现一半的情况进行一下容斥。
...
启发式合并高度权值树。
* [洛谷 P4585] [FJOI2015]火星商店问题: 线段树分治第一弹。
基本照[题解](https://blog.chenqiqian.com/20190402-FJOI2015-shop/)
抄的。
...
### 06/30
* [洛谷 P4559] [JSOI2018]列队: 不难发现对每次命令
将编号 $[l,r]$ 内的学生按位置排序，依次跑向相应位置
的安排是最优的。进一步分析知道需要找到位置 $p$ 使得
位置大于 $p$ 的学生都往右跑，小于 $p$ 的学生都往左跑。
用主席树维护区间计数与坐标和。
* [洛谷 P3369] 【模板】普通平衡树: 用SBT搞了一波。
...
边的管辖范围。
* [洛谷 P1731] [NOI1999]生日蛋糕: 搜索不难写，
剪枝很关键。
* [洛谷 P4588] [TJOI2018]数学计算: 用了中国剩余
定理来处理除的操作。看题解发现正解是线段树。
...
D的边，对于第二类约束，连一条B到A，权值为-D的边。
跑一遍Bellman顺便判断一下负环即可。
...
段线性函数 $a_i x + b_i$ 组成，且一定单调。
且每执行一条指令后新增或减的线段一定在两端。
可用双端队列来维护。对 $a$ 和 $b$ 的增减操作
可用前缀和来优化。乘法操作因为会很快造成溢出，
可以暴力处理。
...
### 05/19
* [洛谷 P4234] 最小差值生成树: LCT，将边按权值排序，
从大到小依次加入，若两端点之间已有路径连通，则先将
路径上的最大边删除。全部连通后维护树上的最大和最小边。
LCT 为了维护边权，可将边转化为点加入树中，转化为点权
的维护。
* [洛谷 P4454] [CQOI2018]破解D-H协议: 离散对数模板题。
被溢出搞得WA两发。
...
令 $A_i^{\prime\prime}=A_i A_{i-1} ... A_0$
考虑第i位每个数的贡献：对于 $[1, A_i-1]$ 内的每个数，
贡献为 $(A_{i+1}'+1)\cdot C_i\cdot(n-i)$
对于 $[A_i+1, B-1]$ 内的每个数
贡献为 $A_{i+1}\cdot C_i\cdot(n-i)$
$A_i$ 的贡献为
$A_{i+1}'\cdot C_i\cdot(n-i) + (A_{i-1}^{\prime\prime}+1)(n-i)\sum_{j=0}^{i}B^j$
...
的特点：每个点只有两种可能性，枚举每个目标位置，
若只有一个原位置可能匹配到它，则建立匹配，并将
原位置的另一可能删除（有可能出现新的唯一匹配的
目标位置，可DFS之）。作完后，若有解，则必定是每
个目标位置有两种匹配可能。直接选择字母序较小的可能
进行第二遍DFS即可。
...
只要同时维护区间和与区间平方和即可。
* [洛谷 P4430] 小猴打架: 先推一下递推公式，
暴力复杂度 $O(n^2)$ 但可以先求出前几项。它们为
1,1,8,96,3000,155520 搜索一下发现通项公式当n大于2
时为 $n^{n-3} n!$ 看题解发现是Cayley定理的推论，
需要学习一下。

### 05/10
* [洛谷 P1263] 宫廷守卫: 转化二分图最大匹配。
匈牙利算法需要卡一下常。
...
分别为123， 132， 213 。二分L，每次判断时枚举所有可能即可。
...
上一个数 $x$ 每个位置的前缀和是 $x$ 的线性函数，所以可预处理
出一个下凸包和一个上凸包，分别对应最大前缀和及最小前缀和，
每次求出两个凸包上对应点的高度差就是答案。

### 04/27
* [洛谷 P3048] [USACO12FEB]牛的IDCow IDs: 枚举找出最高位
的1在哪里，然后递归求解 $(n', k-1)$ 的问题。
* [洛谷 P4514] 上帝造题的七分钟: 前缀和 + 二维树状数组。
对于 $(x_1, y_1, x_2, y_2)$ 范围内数值为 $a$ 的更新，可将其
转化为4个起点为 $(1, 1)$ 的更新，对于每个 $(1, 1, x, y)$ 范围
数值为 $a$ 的更新，用二维数组维护四个量 $a, a x, a y, a x y$
的前缀和，对于每个查询，可找出此区域，区域右方，上方，右上方
对其的贡献之和。用了无耻O2。
* [洛谷 P2822] 组合数问题: 水题，帕斯卡三角 + 前缀和。

### 04/26
* [洛谷 P3978] [TJOI2015]概率论: 神题，看题解才明白，代码
只有一行。知道了一个结论：大小为 $n$ 的不同二叉树的叶子节点
的个数和为 $nC_{n-1}$ $C$ 为Catalan数。
* [洛谷 P4068] [SDOI2016]数字配对: 若 $i$ 和 $j$ 可配对，则在
他们之间连一条边。仔细考虑不难发现这是一个二分图，剩下的就是
二分 + 最小费用最大流。
* [洛谷 P3958] 奶酪: 水题，DFS。
...
剩余元素入队。
* [洛谷 P4113] [HEOI2012]采花: 又是一道看题解才会作的神题，
利用树状数组和next数组，可以只扫描左端点，用树状数组作区间
查询。
...
不得已O2之。
* [洛谷 P3069] [USACO13JAN]牛的阵容Cow Lineup: 其实就是要求
血统数为 $K+1$ 的连续区间内单一血统的最大值。用双指针维护
区间即可。
...
出现的频率， $P(i,j)$ 表示第 $i$ 到第 $j$ 块的众数
（这个比较难想）。

### 04/11
* [洛谷 P2514] [HAOI2010]工厂选址: 枚举 + 贪心。枚举
新厂址，将 $m$ 个旧厂与新厂的运费差排序，将运费差最小的
优先供应旧厂直到满足容量。

### 04/10
* [洛谷 P2518] [HAOI2010]计数: 从高位到低位，每个位置
枚举0到 $s_i-1$  计算剩余部分的组合数累加即可。注意结果
较大，可求出对若干质数的余数，再用中国剩余定理求出结果。

### 04/09
* [洛谷 P2572] [SCOI2010]序列操作: 练习线段树的好题。
两个懒标记和若干统计量，想清楚合并的逻辑即可。

### 04/08
* [洛谷 P3747] [六省联考2017]相逢是问候: 看了题解，
解法是P4139+P4145，需要特别注意的优化是分块存储加速
快速幂。
...
* [洛谷 P2458] [SDOI2006]保安站岗: 树上DP。令
$dp(u, cf, cu)$ 表示 $u$ 的父节点被覆盖情况为 $cf$
$u$ 自身被覆盖情况为 $cu$ 时以 $u$ 为根的子树的最小
费用。当 $cf$ 和 $cu$ 均为0时对所有子节点作一次DP，
找出至少有一个子节点被覆盖时的最小费用。
* [洛谷 P4139] 上帝与集合的正确用法: 扩展欧拉定理
+ 快速幂。 令 $x$  表示所求量，有
$x\mod p = 2^x \mod p = 2^{x\mod \phi(p) + \phi(p)}\mod p$
递归求解 $x\mod \phi(p)$ 最终 $\phi$ 会变成1。

### 04/05
* [洛谷 P3401] 洛谷树: 树链剖分。将每一个二进制位
单独处理。线段树每个区间维护以下统计量： $x$ 终点
为右端点的子区间异或和为1的数目， $y$ 终点为左端点的
子区间异或和为1的数目， $z$ 所有子区间异或和为1的
数目， $s$ 区间内所有数的异或和。在LCA处合并时注意
次序。
...
### 04/02
* [洛谷 P4294] [WC2008]游览计划: 斯坦纳树，注意路径
长度的定义，避免重复计算。树的构建可在DP时进行。

### 03/30
* [洛谷 P4027] [NOI2007]货币兑换: 毒瘤题，调了一天
才搞定。设A的数目为X轴，B的数目为Y轴，则对于每一天
A和B的可能的数目可以维护一个上凸包。每天的可能操作
有现金买入AB劵，或将凸包与斜率为 $-A_i/B_i$ 的直线
的切点沿该直线移动（不得越过直线 $y=\frac{1}{r}x$
）。偷懒用了pb_ds平衡树维护凸包，代码常数较大，不
得不吸氧(O2)。注意精度问题。
...
标记下放时的次序很关键：先更新统计量，再合并懒标记。

### 03/27
* [洛谷 P3706] [SDOI2017]硬币游戏: 开始低估了这题的
难度，建个AC自动机，在上面求出概率分布的稳态。第一个
问题是到达稳态的步数随问题规模增加很快，第二个问题是
问题规模很大时数值稳定性很差，求出都是零解。看题解
发现是高斯消元，关键在于建立一个非终态并找出通过它
转移到各终态的方程。[这里](https://mogicianevian.github.io/2018/03/15/NKOJ-4087-%EF%BC%88SDOI-2017%EF%BC%89%E7%A1%AC%E5%B8%81%E6%B8%B8%E6%88%8F%EF%BC%88%E9%AB%98%E6%96%AF%E6%B6%88%E5%85%83%EF%BC%89/)
讲的比较清楚。

### 03/25
* [洛谷 CF17C] Balance: 仔细观察可发现所有生成的
字符串都是由原字符串的子序列扩展而成。先用DP求出
所有没有相邻重复字符的子序列。令 $dp(i,n_1,n_2,n_3)$
表示终止位置在 $i$ a,b,c数目分别为 $n_1,n_2,n_3$ 的
子序列个数，再计算可由此扩展出的平衡字符串的个数。
...
位置的后续位置，由此可对0到9999的每个数字在 $O(1)$
时间内判断出是否是此运动序列的合法序列。统计出在所
有运动序列中都合法的数字即可。
* [洛谷 CF660C] Hard Process: 双指针。
...
解决。令 $dp(s,i,j)$  表示从节点 $i$ 到节点 $j$
路径节点集合为 $s$ （包括 $i$ 和 $j$ ）的最短
哈密尔顿路径长度。需要作预处理去重以及保证没有字符串
是其他字符串的子串。找字典序最小的解稍微麻烦一些，
需要在解空间DFS 一下。题目所给数据范围不准确导致
RE一发。
...
维护三个距离，城墙距离（最快到达城墙的时间）$d_1$ ，结界
距离（消灭掉所有该城的结界发生器的最快时间）$d_2$ ，入城
距离（可进入该城的最快时间）$d_3$ 。考虑路径连通关系生成
的图为 $g_1$ ,结界连通关系生成的图为 $g_2$ ,有
$d_3(u) = \max(d_1(u), d_2(u))$
$d_1(u) = \min_{e_{vu}\in g_1} (d_3(v) + e_{vu})$
$d_2(u) = \max_{e_{vu}\in g_2} (d_3(v))$
用线段树维护 $d_3$  以便快速更新 $d_2$ ,SPFA进行松弛即可。
...
$f$  只与最后公布成绩的时间 $x$  有关，经分析可知
$f(x)$ 是一下凸函数，故可用三分法求极值。注意特判
$C=10^{16}$  的情况。
...
看了题解，没想出来比较遗憾，想到了二分，
但没想到二分时用最大最小值来分区。
* [洛谷 P1354] 房间最短路问题:计算几何题，
模拟即可。每面墙两个缺口有四个端点，对于
每个端点用它左方所有未被挡住的端点进行
距离松弛。
...
图论 + 启发式合并。先建图，若颜色段
$a_i$ 后接颜色段 $a_j$ 则从 $a_i$ 到
$a_j$ 连边，权重为1。换颜色时将当前
结果减去 $x$ 和 $y$ 代表的节点间的总权重，
再进行启发式合并，将出入度总和较小的节点
合并到出入度总和较大的节点。
...
$b_j$ 的限制条件可由和矩阵奇偶前缀和之差 $c_{ij}$
$a_0$ 和 $a_j$ 给出
$c_{ij}+(-1)^{i+j+1}a_0+(-1)^i a_j + (-1)^j b_i \in [0, P-1]$
以此剪枝即可。
...
严格的证明。
* [洛谷 P1128] [HNOI2001]求正整数: 令
$dp(i,r)$ 表示用第 $i$ 个及以后的质数生成因子
数目为 $r$ 的最小整数，有
$dp(i,r)=\min_{k|r}{p_i}^{k-1}\cdot dp(i+1,r/k)$

### 02/02
* [洛谷 P3222] [HNOI2012]射箭: 设抛物线为
$a x^2 + bx$ 则每一关可表示为在 $a, b$ 平面上
的一对线性约束，二分判断交集是否为空。每次将
直线按斜率排序后求凸包。
...
表示第 $i$ 个区间经 $2^k$ 个不相交区间到达的最近区间。
这样就可快速求出 $[l, r]$ 内最多不相交区间数。按字典序
枚举区间看是否可行即可。

### 01/26
* [洛谷 P3164] [CQOI2014]和谐矩阵:
异或高斯消元第一弹。
* [洛谷 P4198] 楼房重建:用了分块算法，
每个块内维护起点出发的单调递增序列。
每次只须修改一个块并对其他块作二分查找。
...
$t$ 到 $t+1$ 的转移矩阵一定以12为周期。
矩阵快速幂求12的整数倍幂次。可预处理出
每个石墩哪些时间是危险的。
...
的余数（二进制表示），被T飞。看题解发现状态是
$dp(i,r)$ 表示前缀 $i$ 余数 $r$ 的最小分块数。
* [洛谷 P2107] 小Z的AK计划: 贪心，用优先队列
维护已选的机房，当时间不够走到下一个机房或
选择当前机房时，根据情况弹出一些耗时最多的机房。
...
将所有元素相乘所得的结果为满足各子矩阵最大
值不大于 $v$ 的方案数。用容斥原理可求出各
子矩阵最大值等于 $v$ 的方案数。

### 01/02
* [洛谷 P3243] [HNOI2015]菜肴制作:看题解才
明白要求反向图上字典序最大的序列。
* [洛谷 P4109] [HEOI2015]定价:算法上没有什么
难点，重点在于实现的细节。
...
体力值为 $h$ 陷阱状态为 $s$ 的最大逃脱几率。
每次从 $(x,y)$ 出发搜索未知或有害的边界点，
下一步转移到这些边界点。

### 12/24
* [洛谷 P2474] [SCOI2008]天平:看题解才
了解到差分约束的做法。
* [洛谷 P3959] 宝藏:类似斯坦纳树的思想，
多加一维，令 $dp(l,r,s)$ 表示子树根为
$r$ 从根到子树根距离为 $l$ 子树内节点
集合为 $s$ 的最小代价，状态转移与斯坦纳树
相似。
...
看题解才会，01背包 + 分组背包。
* [洛谷 P2573] [SCOI2012]滑雪:没想到在
Kruskal 里按高度排序，以为要用最小树型图，
如此看来若有向图中边的方向性可由点的势函数
决定，可由Kruskal来解决最小树型图问题。

### 12/21
* [洛谷 P3622] [APIO2007]动物园:状压DP，令
$dp(i,mask)$ 表示进行到位置 $i$ $[i,i+4]$
的动物移除状态为 $mask$ 的最优解。因为是
环形结构，枚举起点可能的32种状态，并动归到
位置 $N-5$ 剩余位置的构型已完全决定了。
重要优化： $dp(i,m)$ 和 $dp(i,m|16)$ 到
$i+1$ 的转移可同时进行。

### 12/20
* [洛谷 P4322] [JSOI2016]最佳团体:分数规划 +
树上背包，DFS序优化。
* [洛谷 P4177] [CEOI2008]order: 类似最大权闭合图，
唯一不同在于任务到机器的连边容量不是无穷，而是
租用费用。
* [洛谷 P3306] [SDOI2013]随机数生成器: 找出
通项公式 $X_i=a^{i-1}(X_1+\frac{b}{a-1})-\frac{b}{a-1}$
用逆元和离散对数可求出 $i$

### 12/19
* [洛谷 P4174] [NOI2006]最大获利: 网络流，
拆点，从源点向 $u$ 连边，容量为与该点有关的
总收入，从 $u'$ 向汇点连边，容量为该点的成本，
从 $u$ 向 $v'$ 连边，容量为相应的 $C$ 值，
答案为两倍的总收入减去最小割的容量，再除以2。

### 12/18
* [洛谷 P3296] [SDOI2013]刺客信条:枚举每个节点
作为根的情况,对于每个节点 $u$ ，递归找到每个子节点
同构于相应原树节点 $u'$ 的哪些子节点，对子节点跑一遍最小
费用流找到 $u$ 是否同构于 $u'$ 以及最小同构代价。
...
* [洛谷 P3299] [SDOI2013]保护出题人: 横坐标为位置，
纵坐标为前缀和作图发现每个回合的攻击值就是最大斜率。
用双端队列维护一个凸包，每次三分查找最大斜率。
...
一下是否所有的回路长度都是0即可。
* [洛谷 P3305] [SDOI2013]费用流:显然B的最佳策略
是将P全部加到流量最大的边上，相应的A的最佳策略
是流量最大的边的流量尽可能小。二分流量上界跑
网络最大流即可。注意A的最佳策略流量有可能是浮点
数。
...
* [洛谷 P3161] [CQOI2012]模拟工厂:看题解才会作，
枚举+贪心，枚举 $2^N$ 种可能的方案，按时间顺序
处理所有选择的订单，对于订单 $i$ 考虑它之后所有
的订单，找出它之后可最多提高生产率多少次，依次
模拟。

### 12/05
* [洛谷 P2217] [HAOI2007]分割矩阵: DP，简单推导可知
目标是最小化 $\sum {A_i}^2$ $A_i$ 是第 $i$ 个子矩阵
的元素和。令 $dp(i_1,j_1,i_2,j_2,n)$ 表示左上角为
$(i_1,j_1)$ 右下角为 $(i_2,j_2)$ 的子矩阵分为 $n$
块的最佳方案。
...
$r\in[2,7]$ 的形式。快速幂+高精。
* [洛谷 P2536] [AHOI2005]病毒检测: Trie上用DFS作匹配，
记忆化搜索避免TLE。

### 12/02
* [洛谷 P4043] [AHOI2014/JSOI2014]支线剧情:有源汇有下界的最小
费用可行流问题。在伴随网络上跑一遍最小费用最大流即可。
* [洛谷 P1402] 酒店之王:网络流，将顾客拆点放中间，房间放左边，
食物放右边，源向房间连边，食物向汇连边，房间和食物按喜好关系
和顾客连边，跑一遍最大流即可。
* [洛谷 P4148] 简单题:KD-tree 第一弹。
* [洛谷 P4555] [国家集训队]最长双回文串:用Manacher算法找出
每个位置开始和结束的最大回文串长度，枚举位置即可。
* [洛谷 P3167] [CQOI2014]通配符匹配:看题解才会作，DP+字符串哈希。

### 11/27
* [洛谷 P2762] 太空飞行计划问题：用类似最大点权闭合图的方法建图。
* [洛谷 P4455] [CQOI2018]社交网络：有向图的矩阵树定理。
...
即可。
* [洛谷 P2754] [CTSC1999]家园：每一天为一层，从每个节点向下一层
...
* [洛谷 P2737] [USACO4.1]麦香牛块Beef McNuggets：枚举+背包。
重要前提结论：对于任何互质的整数 $p$ 和 $q$ 不能用他们表示的
最大整数为 $pq-p-q$

### 11/21
* [洛谷 P2761] 软件补丁问题：状态压缩 + SPFA求最短路。不知为什么
归到网络流24题里，搞得想岔了半天。
* [洛谷 P2770] 航空路线问题：转化为有向图的点连通度问题。用最小
费用最大流跑一遍起点到终点的点连通度，看是否大于1。
* [洛谷 P1215] [USACO1.4]母亲的牛奶 Mother's Milk：试炼场水题，DFS搞搞。
...
DP求最大正方形。
* [洛谷 P2744] [USACO5.3]量取牛奶Milk Measuring：IDDFS。
* [洛谷 P2745] [USACO5.3]窗体面积Window Area：数据范围不大，
暴力搞搞即可。求覆盖面积用了暴力DFS。
* [洛谷 P4215] 踩气球：将左右端点分别当作x,y坐标，则每个区间
转化为平面上的一个点。当任一个箱子x变空时，找到此空箱子所在的连续
空箱子区间的左右端点L，R。则此次操作使高兴的熊孩子数为x坐标在
$[L,x]$ y坐标在 $[x,R]$ 的点数。比标程多了 $\log N$ 的因子，
不吸氧的话会T掉两个点。
...
看题解发现二项式反演有更好的时间复杂度。
* [洛谷 P3949] 答案错误：令1表示X，-1表示Z，可推理证明若
$x\in[2^{n-1}-1, 2^n]$ 则 $f(x)=-f(x-2^{n-1})$
并有 $f(1)=1$ 递归求解即可。
* [洛谷 P3359] 改造异或树：看了题解，关键点：每个点赋一个异或值 +
启发式合并。
* [洛谷 P2841] A*B Problem：以为是数学题，看了题解发现是搜索题。

### 11/09
* [洛谷 P3358] 最长k可重区间集问题：最小费用最大流。
* [洛谷 P2045] 方格取数加强版：同上。
* [洛谷 P3365] 改造二叉树：中序遍历的数组 $A$ 类似P2501的思想，
令 $B[i]=A[i]-i$ 求 $B$ 的最长不降序列长度。遇到谜之RE，似乎
数据有问题。
