...
public:
    ...
    static BigInt mul(const BigInt& a, const BigInt& b) {
        ...
        if (ret.splits.size() > 56) {
            ret.splits.erase(ret.splits.begin() + 56, ret.splits.end());
        }
        return ret;
    }
...
BigInt fastPower(const BigInt& x, int p) {
    if (p == 1) return x;
    BigInt y = fastPower(x, p/2);
    BigInt ans = y * y;
    if (p & 1) ans = ans * x;
    return ans;
}
...
int main() {
    ...
    cin >> p;
    BigInt two("2");
    BigInt ans = fastPower(two, p);

    int numDigits = 0.301029995663 * p + 1;
    printf("%d\n", numDigits);

    string ret = ans.toString();
    int L = ret.length();
    if (L > 500) {
        ret = ret.substr(L-500, 500);
    } else {
        ret = string(500-L, '0') + ret;
    }
    ret[499]--;

    for (int i = 0; i < 10; i++) {
        printf("%s\n", ret.substr(i*50, 50).c_str());
    }
    return 0;
}
