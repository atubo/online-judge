// https://www.luogu.org/problemnew/show/P1128
// [HNOI2001]求正整数
...
int N;
map<pair<int, int>, BigInt> dp;

BigInt solve(const vector<int> &primes, int pi, int r) {
    if (r == 1) return 1;
    auto key = make_pair(pi, r);
    if (dp.count(key) > 0) return dp[key];
    BigInt& ret = dp[key];
    for (int i = 2; i <= r; i++) {
        if (r % i == 0) {
            auto tmp = qpow(primes[pi], i-1) * solve(primes, pi+1, r/i);
            if (ret == 0 || tmp < ret) {
                ret = tmp;
            }
        }
...
int main() {
    ...
    auto ret = solve(sieve.primes, 0, N);
