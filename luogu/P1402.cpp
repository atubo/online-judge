...
int main() {
    scanf("%d%d%d", &N, &P, &Q);
    Dinic dinic(P+2*N+Q+2, P+P*N+N+N*Q+Q);
    const int src = 0, dst = P+2*N+Q+1;
    for (int i = 1; i <= N; i++) {
        dinic.addEdge(P+2*i-1, P+2*i, 1);
    }
    for (int p = 1; p <= P; p++) {
        dinic.addEdge(src, p, 1);
    }
    for (int q = 1; q <= Q; q++) {
        dinic.addEdge(P+2*N+q, dst, 1);
    }
    for (int i = 1; i <= N; i++) {
        for (int p = 1; p <= P; p++) {
            int x;
            scanf("%d", &x);
            if (x) dinic.addEdge(p, P+2*i-1, 1);
        }
    }
    for (int i = 1; i <= N; i++) {
        for (int q = 1; q <= Q; q++) {
            int x;
            scanf("%d", &x);
            if (x) dinic.addEdge(P+2*i, P+2*N+q, 1);
        }
    }
    printf("%d\n", dinic.dinic(src, dst));
    return 0;
