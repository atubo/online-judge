// https://www.luogu.org/problemnew/show/P2223
// [HNOI2001]软件开发
...
int main() {
    ...
    A++; B++;
    MincostMaxflow flow(2*N+2, 6*N);
    const int src = 0, dst = 2*N+1;
    for (int i = 1; i <= N; i++) {
        int x;
        scanf("%d", &x);
        flow.addEdge(src, top[i], F, MincostMaxflow::INF);
        flow.addEdge(top[i], dst, 0, x);
        flow.addEdge(src, bot[i], 0, x);
        int j = i + A;
        if (j <= N) flow.addEdge(bot[i], top[j], FA, MincostMaxflow::INF);
        j = i + B;
        if (j <= N) flow.addEdge(bot[i], top[j], FB, MincostMaxflow::INF);
        if (i < N) flow.addEdge(bot[i], bot[i+1], 0, MincostMaxflow::INF);
    }
    auto ret = flow.mincostFlow(src, dst);
    printf("%ld", ret.second);
