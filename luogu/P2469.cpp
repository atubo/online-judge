// https://www.luogu.org/problemnew/show/P2469
// [SDOI2010]星际竞速
...
int upper(int i) {return 2*i-1;}
int lower(int i) {return 2*i;}
...
int main() {
    ...
    MincostMaxflow maxflow(T+1);
    for (int i = 1; i <= N; i++) {
        scanf("%d", &A[i]);
        int u = upper(i);
        int v = lower(i);
        maxflow.addEdge(S, u, 0, 1);
        maxflow.addEdge(v, T, 0, 1);
        maxflow.addEdge(S, v, A[i], 1);
    }
    ...
    for (int i = 0; i < M; i++) {
        ...
        scanf("%d%d%d", &u, &v, &w);
        if (u > v) swap(u, v);
        maxflow.addEdge(upper(u), lower(v), w, 1);
    }

    pair<int, int> result = maxflow.mincostFlow(S, T);
    printf("%d\n", result.second);
