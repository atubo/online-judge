...
int solve(MincostMaxflow& flow, int u, int v) {
  flow.init();
  for (int i = 0; i < N; i++) {
    flow.addEdge(0, i+1, 0, 1);
    flow.addEdge(i+N+1, 2*N+1, 0, 1);
    for (int j = 0; j < N; j++) {
      if (i == u && j == v) continue;
      flow.addEdge(i+1, j+N+1, -H[i][j], 1);
    }
  }
  auto f = flow.mincostFlow(0, 2*N+1);
  return -f.second;
}
...
int main() {
  ...
  MincostMaxflow flow(2*N+2, N*N+2*N);
  for (int i = 0; i < N; i++) {
    flow.addEdge(0, i+1, 0, 1);
    flow.addEdge(i+N+1, 2*N+1, 0, 1);
    for (int j = 0; j < N; j++) {
  ...
  for (int i = 0; i < N; i++) {
    if (solve(flow, i, matched[i]) < -f.second) {
      printf("%d %d\n", i+1, matched[i]+1);
    }
  }
