...
int64_t solve(int factor) {
    const int src = 0, snk = M+N+1;
    MincostMaxflow flow(snk+1, M+N+M*N);
    for (int i = 1; i <= M; i++) {
        flow.addEdge(src, i, 0, A[i]);
        for (int j = 1; j <= N; j++) {
            flow.addEdge(i, M+j, C[i][j]*factor, MincostMaxflow::INF);
        }
    }
    for (int i = 1; i <= N; i++) {
        flow.addEdge(M+i, snk, 0, B[i]);
    }

    auto ret = flow.mincostFlow(src, snk);
    return ret.second * factor;
}
...
int main() {
    ...
    int64_t ans1 = solve(1);
    int64_t ans2 = solve(-1);
    printf("%ld\n%ld\n", ans1, ans2);
    return 0;
}
