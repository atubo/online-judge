...
public:
    ...
    int64_t queryNode(int u) {
        int64_t ret = queryEdge(u, 1);
        int r = stIdx[1];
        ret += st.query(r, r);
        return ret;
    }
...
int main() {
    ...
    for (int i = 0; i < Q; i++) {
        int l, r, z;
        scanf("%d%d%d", &l, &r, &z);
        queries[l].push_back({0, i, z+1});
        queries[r+1].push_back({1, i, z+1});
    }
    for (int i = 1; i <= N; i++) {
        hld.updateNode(i);
        for (auto q: queries[i]) {
            int64_t res = hld.queryNode(q.z);
            if (q.op == 0) ans[q.id] -= res;
            else ans[q.id] += res;
        }
    }
    for (int i = 0; i < Q; i++) {
